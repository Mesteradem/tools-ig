	...
	try:
		...
		with requests.Session() as ses_dev:
			...
			if "userId" in str(data_dev):
				...
				print" %s[!] aktifna mode pesawat 2 detik!"%(M)
...
def logo():
	...
	print("""%s
\x1b[1;94m ╔═════════════════╗  
\x1b[1;94m   BY.SAMBATX TEAM    
\x1b[1;94m                       
\x1b[1;94m code by aditya team   
\x1b[1;94m ╚═════════════════╝    """%(N))
...
def menu_dev():
	...
	print(" [*] ---------------------------------------------")
	print(" [*] IP          : %s\n"%(IP))
	print(" [ selamat datang %sMEMBER SAMBATX%s ]\n"%(K,N))
	print(" [01]. hack kan akun publik")
	print(" [02]. hack kan pencarian")
	print(" [03]. hack ulang hasil cp")
	print(" [04]. hack akun hasil crack")
	...
	if pil == "1" or pil == "01":
		pas = ""
		status = ""
		username = raw_input(" [?] lebokna username target : ")
		info_dev(username, pas, status)

		print("\n [ milih target salah siji ]")
